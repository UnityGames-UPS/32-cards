...
public class BetHistoryItemView : MonoBehaviour
{
    ...
    public void Populate(int globalNumber, BetHistoryEntry entry)
    {
        ...
        if (stakeText != null)
            stakeText.text = GameUtility.FormatCurrency(entry.bet_amount);

        if (winText != null)
            winText.text = GameUtility.FormatCurrency(entry.win_amount);

        if (netText != null)
        {
            double net = entry.win_amount - entry.bet_amount;
            string sign = net >= 0 ? "+" : "-";
            netText.text = sign + GameUtility.FormatCurrency(Math.Abs(net));
        }
